...
import { Plus, X, Target, Ban } from "lucide-react";
import toast from "react-hot-toast";
import { LifeHabit } from "@/types";
...
export default function NewLifeChapterPage() {
  ...
  const addHabit = () => {
    ...
    if (habits.length >= 6) {
      toast.error("Maximum 6 habits per Life Chapter");
      return;
    }

    const habit: LifeHabit = {
  ...
  const createLifeChapter = async () => {
    ...
    if (duration < 30) {
      toast.error("Minimum duration is 30 days");
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        toast.success("Life Chapter created! Your transformation begins now 🔥");
        router.push("/dashboard");
      } else {
        const data = await res.json();
        toast.error(data.error || "Failed to create Life Chapter");
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="mb-8">
          <h1 className="text-3xl font-bold text-white mb-2">Create Life Chapter</h1>
          <p className="text-gray-400">Design your transformation journey with connected habits</p>
        </div>

        <div className="space-y-8">
          {/* Basic Info */}
          <Card>
            <h2 className="text-xl font-bold text-white mb-4">Chapter Details</h2>
          ...
          <div className="flex justify-end gap-4">
            ...
            <Button
              onClick={createLifeChapter}
              disabled={loading}
              className="flex items-center gap-2"
            >
              {loading ? "Creating..." : "Create Life Chapter"}
            </Button>
          </div>
        </div>
      </main>
